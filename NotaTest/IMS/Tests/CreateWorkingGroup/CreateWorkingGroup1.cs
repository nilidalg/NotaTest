using System;
using System.IO;
using System.Text;
using System.Collections.Generic;
using System.Text.RegularExpressions;
using System.Windows.Input;
using System.Drawing.Imaging;
using OpenQA.Selenium;
using OpenQA.Selenium.Chrome;
using OpenQA.Selenium.Support.UI;
using SeleniumExtras.PageObjects;
using NUnit.Framework;
using System.Linq;
using System.Configuration;
using OpenQA.Selenium.Remote;
using System.Diagnostics;
using NotaTest.IMS.PageObjects;
using NotaTest.IMS.Login;
using System.Threading;
using RelevantCodes.ExtentReports;
using NUnit.Framework.Interfaces;

namespace IMS.Tests.CreateWorkingGroup1
{
    public class CreateWorkingGroup1
    {
        private IWebDriver driver;
        private WebDriverWait wait;
        private LoginObjects loginObjects;
        private GoTo goTo;
        private WorkingGroup workingGroup;
        public static ExtentTest test;
        public static ExtentReports extent;



        [SetUp]

        public void Test()
        {
            string path = System.Reflection.Assembly.GetCallingAssembly().CodeBase;
            string actualPath = path.Substring(0, path.LastIndexOf("bin"));
            string projectPath = new Uri(actualPath).LocalPath;
            string reportPath = projectPath + "Reports\\MyOwnReport.html";

            extent = new ExtentReports(reportPath, false);
            extent
            .AddSystemInfo("Host Name", "Artur")
            .AddSystemInfo("Environment", "QA")
            .AddSystemInfo("User Name", "Artur G");
            extent.LoadConfig(projectPath + "extent-config.xml");

            driver = new ChromeDriver();
            wait = new WebDriverWait(driver, TimeSpan.FromSeconds(3000));
            driver.Manage().Window.Maximize();
            loginObjects = new LoginObjects(driver);
            goTo = new GoTo(driver);
            workingGroup = new WorkingGroup(driver);


        }

        [Test]
        public void TestCreateWorkingGroup1()
        {
            test = extent.StartTest("Запуск БП Создание рабочей группы ");
            goTo.LoginPage(ConfigurationManager.AppSettings["LoginPage"]);
            loginObjects.InputLoginOperReg(ConfigurationManager.AppSettings["LoginOperReg"]);
            loginObjects.InputPwdOperReg(ConfigurationManager.AppSettings["PwdOperReg"]);
            loginObjects.LoginButton();
            goTo.WorkingGroupPage(ConfigurationManager.AppSettings["WorkingGroupPage"]);
            workingGroup.AddWorkingGroupButton();
            workingGroup.NumberOfGroupInput();
            workingGroup.DescriptionOfGroupInput();
            workingGroup.ShortDescriptionOfGroupInput();
            workingGroup.AdressOfGroupInput();
            workingGroup.LevelOfGroupInput();

            workingGroup.LevelOfGroupInputValueReg();
            System.Threading.Thread.Sleep(3000);
            workingGroup.PriorityOfGroup();
            System.Threading.Thread.Sleep(3000);
            workingGroup.PriorityOfGroupInputMedium();

            workingGroup.BigDescriptionOfGroupInput();
            workingGroup.SaveButton();

            test.Log(LogStatus.Pass, "Тест пройден успешно");
        }

        [TearDown]
        public void GetResult()
        {
            var status = TestContext.CurrentContext.Result.Outcome.Status;
            var stackTrace = "<pre>" + TestContext.CurrentContext.Result.StackTrace + "</pre>";
            var errorMessage = TestContext.CurrentContext.Result.Message;

            if (status == TestStatus.Failed)
            {
                test.Log(LogStatus.Fail, stackTrace + errorMessage);
            }
            extent.EndTest(test);
            driver.Quit();
            driver = null;

        }

        [OneTimeTearDown]
        public void ExtentClose()
        {
            extent.Flush();
            extent.Close();
        }
    }
}
