...
namespace IMS.Tests.Incident3_2
{
    public class Incident3_2
    {
        ...
        [Test]
        public void TestIncident3_2()
        {
            test = extent.StartTest("Создание инцидента федерального уровня");
            goTo.LoginPage(ConfigurationManager.AppSettings["LoginPage"]);
            loginObjects.InputLoginSoglFed(ConfigurationManager.AppSettings["LoginSoglFed"]);
            loginObjects.InputPwdSoglFed(ConfigurationManager.AppSettings["PwdSoglFed"]);
            loginObjects.LoginButton();
            goTo.IncidentPage(ConfigurationManager.AppSettings["IncidentPage"]);
            incidentPage.CheckCreatIncidentButton();
